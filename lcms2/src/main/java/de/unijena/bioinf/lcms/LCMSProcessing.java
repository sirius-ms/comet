            ...
            jobs.add(SiriusJobs.getGlobalJobManager().submitJob(new BasicJJob<long[]>() {
                ...
                protected long[] compute() throws Exception {
                    ...
                    if (mergedTrace!=null && isSuitableForImport(mergedTrace)) {
                        return Arrays.stream(importer.importMergedTrace(mergedTraceSegmentationStrategy, siriusDatabaseAdapter, obj,merged, mergedTrace, tag)).mapToLong(AlignedFeatures::getAlignedFeatureId).toArray();
                    }
                    return new long[0];
                }
            }));
